...
import TeacherAccessLog from '#models/teacher_access_log'
...
export default class Teacher extends compose(BaseModel, AuthFinder) {
  ...
  static accessTokens = DbAccessTokensProvider.forModel(Teacher, {
    expiresIn: '30 days',
    prefix: 'oatTeacher_',
   ...
   */
  @beforeCreate()
  static async assignUuid(item: Teacher) {
    item.uuid = uuidv4()
  }

  @beforeSave()
  static async hashPasswordHook(item: Teacher) {
  ...
  @hasMany(() => TeacherAccessLog)
  declare teacherAccessLogs: HasMany<typeof TeacherAccessLog>
